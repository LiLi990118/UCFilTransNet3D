...
import numpy as np
from networks.Cotr import CNNBackbone
from networks.Cotr.DeformableTrans import DeformableTransformer
from networks.Cotr.position_encoding import build_position_encoding
...
class U_ResTran3D(nn.Module):
    def __init__(self, norm_cfg='BN', activation_cfg='ReLU', num_classes=8, weight_std=False):
        ...
        self.encoder_Detrans = DeformableTransformer(d_model=384, dim_feedforward=1536, dropout=0.1, activation='gelu',
                                                     ...
                                                     enc_n_points=4)
        self.l1 = nn.Linear(3888,432)
        self.l2 = nn.Linear(3888, 3456)
    ...
    def forward(self, inputs):
        ...
        #Single_scale
        # x = self.transposeconv_stage2(x_convs[3])
        # skip2 = x_convs[-2]
        # Multi-scale
        x = self.l1(x_trans.transpose(-1, -2))
        x = self.transposeconv_stage2(x.view(x_convs[-1].shape))
        skip2 = self.l2(x_trans.transpose(-1, -2)).view(x_convs[-2].shape)
...
if __name__ == '__main__':
    ...
    x = Variable(torch.rand(1, 1, 96, 96, 96)).cuda()
    model = U_ResTran3D(num_classes=5).cuda()
